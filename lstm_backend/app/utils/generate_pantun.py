...
# ====== GENERATE PANTUN ======
def generate_pantun(model, seed_text, word2idx, idx2word, max_words=100, target_baris=4, device='cpu'):
    model.eval()
    words = seed_text.lower().split()
    br_count = 0

    for _ in range(max_words):
        token_ids = [word2idx.get(w, word2idx.get('<UNK>', 0)) for w in words]
        padded = [0] * (50 - 1 - len(token_ids)) + token_ids  # asumsi max_seq_len = 50
        ...
        if next_word.lower() == '<br>':
            br_count += 1
            if br_count >= target_baris:
                break

    full_text = ' '.join(words)
    baris = [b.strip() for b in full_text.lower().split('<br>') if b.strip()]

    return '\n'.join(baris[:target_baris])